...
void TilePress(){
    ...
    Serial.println((5*SPACE_BETWEEN + 4*TILE_SIZE));
    delay(500);
    if (touch.z < MIN_PRESSURE || touch.z > MAX_PRESSURE){
        TilePressed = 0;
    }
    else if (touch.x < 5 * SPACE_BETWEEN + 4*TILE_SIZE){
        TilePressed = ((((touch.x)-5)/50 + 1) + 4*(((touch.y)-5)/50));
        Serial.print("TilePressed:");
